...
//       console.log (animations);
      ...
      this.fadeToAction ("Running", true, 1.2);
...
//     var that = this;
  ...
  fadeToAction (name, repeat, speed) {
    ...
    this.activeAction.crossFadeFrom (previousAction, this.activeAction.time/60 )
  ...
  saltar(){
    ...
    var origen = {p : 0};
    var destino = {p : 0.3};
    var bajar = new TWEEN.Tween(origen).to(destino,300).easing(TWEEN.Easing.Cubic.In).onUpdate(()=>{
      if(this.position.y > 0)
        this.position.y-=origen.p;
    }).onComplete(()=>{
      if(this.position.y > 0)
        bajar.start();
      else{
        if(!muerto)
          this.fadeToAction ("Running", true, 1.2);saltando = false;
        
      }
        
    });

    var salto = new TWEEN.Tween(origen).to(destino,400).easing(TWEEN.Easing.Back.Out).onUpdate(()=>{
      var dt = this.clock.getDelta();
      if(this.position.y < 5)
      this.position.y+=origen.p;

    }).onStart(()=>{this.fadeToAction ("Jump", false, 1.1);saltando=true;}).chain(bajar);
    if(true){
      muerto = false;
      salto.start();
    }
  ...
  morir(){
    ...
    var bajar = new TWEEN.Tween(origen).to(destino,300).easing(TWEEN.Easing.Cubic.In).onUpdate(()=>{
      ...
      if(this.position.y > 0)
        ...
        this.fadeToAction ("Death", false, 1);muerto = true;
    });

    /* if(this.position.y > 0){}
     // bajar.start();
    else{
      this.fadeToAction ("Death", false, 1);saltando = false;
    } */
    this.fadeToAction ("Death", false, 1);muerto = true;
  ...
  update (pos) {
    ...
    //fadeToAction ("Jump", true, 1.4);
    var dt = this.clock.getDelta();
    if (this.mixer) this.mixer.update (dt);

    
    
    this.position.x=THREE.Math.lerp(this.position.x,pos*-6, 5.5*dt);
